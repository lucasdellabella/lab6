...
class CozMap:
    ...
    def compute_smooth_path(self):
        ...
        def get_rand_index_in_path(path):
        ...
        for _ in range(25):
            ...
            i, j = -1, -1
            while i >= j:
                i, j = get_rand_index_in_path(path), get_rand_index_in_path(path)
            node0, node1 = path[i], path[j]

            ## Ensures that a path between these two is legit
            if not self.is_collision_with_obstacles((node0, node1)):
